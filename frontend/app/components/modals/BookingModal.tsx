...
const BookingModal: React.FC<BookingModalProps> = ({
  ...
  const handlePayment = async () => {
    ...
    // Double-check that user is not the owner
    if (ownerEmail === (user?.email || user?.emailId)) {
      toast.error("You cannot book your own item!");
      setIsLoading(false);
      return;
    }

    // Validate maximum amount (₹5,00,000 limit for test environment)
    const maxAmountInRupees = 500000;
    if (bookingData.totalAmount > maxAmountInRupees) {
      toast.error(`Total amount ₹${bookingData.totalAmount.toLocaleString()} exceeds maximum allowed amount of ₹${maxAmountInRupees.toLocaleString()}. Please reduce the rental duration or daily rate.`);
      setIsLoading(false);
      return;
    }
        ...
        <div className="p-6">
          ...
          {step === 'details' && (
            <div className="space-y-6">
                      ...
                      <div className="mt-2 p-2 bg-red-50 border border-red-200 rounded-lg">
                        <div className="flex items-center">
                          ...
                          <p className="text-sm text-red-700">
                            Amount exceeds ₹1,00,000 limit. Please reduce rental duration.
                          </p>
                        </div>
                      </div>
                    )}
              ...
              <button
                onClick={handleNext}
                disabled={!bookingData.startDate || !bookingData.endDate || bookingData.totalDays <= 0 || bookingData.totalAmount > 500000}
              ...
              >
                {bookingData.totalAmount > 500000 ? 'Amount Too High' : 'Proceed to Payment'}
              </button>
            </div>
          )}
