...
import MultiCategoryInput from "../inputs/MultiCategoryInput";
...
const RentModal = () => {
  ...
  const onSubmit: SubmitHandler<FieldValues> = (data) => {
    ...
    const itemData = {
      ...
      price: data.price,
      category: data.category || [], // Now expects an array of categories
    ...
    console.log("Category:", data.category, "Type:", typeof data.category, "Length:", data.category?.length);
    ...
    if (!data.category || data.category.length === 0) missingFields.push('category');
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="text-center">
        <h2 className="text-2xl font-bold text-alibaba-black mb-2">
          Choose Your Item Categories
        </h2>
        <p className="text-alibaba-gray-600 mb-4 text-base">
          {description || "Select one or more categories that best describe your item"}
        </p>
      </div>
        ...
        {categories.map((item) => (
          <div key={item.label} className="col-span-1">
            <MultiCategoryInput
              ...
              selected={category && category.includes(item.label)}
              label={item.label}
              icon={item.icon}
            />
          </div>
      ...
      {category && category.length > 0 && (
        <div className="mt-4 p-3 bg-green-50 border border-green-200 rounded-lg">
          <p className="text-green-800 text-sm font-medium mb-2">Selected Categories:</p>
