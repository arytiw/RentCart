...
export async function POST(request: NextRequest) {
  try {
    ...
    // Create nodemailer transporter (using Gmail for demo - configure with your email service)
    const transporter = nodemailer.createTransport({
      service: 'gmail',
      auth: {
        user: process.env.EMAIL_USER || 'rentcart.service@gmail.com',
        pass: process.env.EMAIL_PASS || 'demo_password_here', // Use app password for Gmail
      },
    });
    ...
    await transporter.sendMail({
      from: '"RentCart" <rentcart.service@gmail.com>',
      to: renterEmail,
